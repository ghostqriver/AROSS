...
def check_len(data):
    '''
    When the length of training and test set are less than 10, it would cause an error in TABGAN
    See: https://github.com/Diyago/GAN-for-tabular-data/blob/master/src/tabgan/sampler.py#L224
    '''
    while len(data) < 10:
            data = data.append(data.iloc[random.randint(0,len(data)-1)])
    return data
...
def run_tabgan(X_train, y_train, X_test,y_test,target,classes):
    ...
    batch_size = 10 # Originally it is 16, which would cause an error
    for c in set(classes):
        training_data=tmp[tmp[target]==c]
        test_data = tmp_test[tmp_test[target]==c]
        print('CLASS',c)
        nb_instances_to_generate=1+max_class/count_classes[c] 
        print()
        if nb_instances_to_generate !=1:
            test_data = check_len(test_data)
            training_data = check_len(training_data)
            ...
            new_tr, new_tar = GANGenerator(gen_x_times=nb_instances_to_generate, cat_cols=None,
                ...
                gan_params = {"batch_size": batch_size, "patience": 5, "epochs" : 150,}).generate_data_pipe(pd.DataFrame(training_data.drop(target,1)), 
                                                                                                pd.DataFrame(training_data[target]), 
                                                                                                test_data.drop(target,1), 
